...
def retrieve_download_links(georegion, user_n, pass_n, start_date , end_date , max_meters, strict_region , max_diff, cloud_coverage_whole):
    """
    ...
    rings = geojson_coordinates[0][0]
    wkt = f"MULTIPOLYGON (((" + ", ".join(f"{lon} {lat}" for lon, lat in rings) + ")))"
    myobj = { "type": "Feature","geometry": wkt, "properties": {"fields":{"geometry":"false"},"filters" :
    { "datefilter":{ "startdate": start_date,"enddate" : end_date }, "resolutionfilter":{ "maxres" :max_meters, "minres" : 0 }} } }
 
    headers = {'content-type': 'application/json'}
    x = requests.post(url, auth = HTTPBasicAuth(user_n, pass_n), json = myobj)  
    ...
    for row in reponse['features']:
        try:  
            if row['properties']['downloads'] is not None:
                ...
                if row['properties']['cloudcover'] is None or float(row['properties']['cloudcover']) < cloud_coverage_whole  :
