...
PRODUCT_EXES = {
    "AllSource": "ArcGISAllSource.exe",
    "Drone2Map": "Drone2Map.exe",
    "GeoScene": "GeoScenePro.exe",
    "Pro": "ArcGISPro.exe",
    "RealityStudio": "ArcGISRealityStudio.exe",
...
def product_install_dir():
    ...
    reg_paths = {
        "AllSource": "SOFTWARE\\Esri\\ArcGISAllSource",
        "Reality": "SOFTWARE\\Esri\\ArcGISRealityStudio",
            ...
            if key:
                try:
                    ...
                    if install_dir.exists():
                        if product == "Pro":
                            # GeoScene shows up as Pro in the registry, check for its executable
                            geoscene_exe = (install_dir / "bin" / PRODUCT_EXES["GeoScene"])
                            if geoscene_exe.exists():
                                product = "GeoScene"
    ...
    if not install_dir or not install_dir.exists():
        raise ImportError(
            "Unable to locate a valid ArcGIS product installation to use with ArcPy. ArcPy must be installed, and match the version of the package used."
        )
...
def set_product_paths():
    ...
    for p in os.environ["PATH"].split(";"):
        path_p = Path(p)
        # exclude duplicates, remove bin entry
        if path_p not in win_path and path_p != bin_dir:
    ...
    # add bin explicitly to DLL load locations
...
def install():
    ...
    try:
        ...
        # silently pass if we can't import lxml. Likely state is during a clone when lxml isn't
        # properly set up against the right libxml2
        return
    ...
    if is_dev:
        # Check if the environment is active in Pro, if so our dest details will be mapped
        # to the current active environment and be incorrect.
        if "PREFIX" in os.environ.keys() and not sys.prefix in sys.executable:
            dest_site_packages = Path(os.environ["PREFIX"]) / "Lib/site-packages"
